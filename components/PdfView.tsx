...
export const PdfView: React.FC<Props> = ({ 
  ...
  const handlePdfClick = (type: 'FREE' | 'PREMIUM' | 'ULTRA') => {
      ...
      if (!link) {
          // AI GENERATION LOGIC
          const promptOverride = syllabusMode === 'COMPETITION' 
              ? (settings?.promptCompetitiveNotes || "") 
              : type === 'PREMIUM' 
                  ? (settings?.promptPremiumNotes || "") 
                  : (settings?.promptFreeNotes || "");
          
          const confirmMsg = type === 'PREMIUM' 
              ? `Notes not found. Generate PREMIUM Notes with AI? (Cost: ${price} CR)` 
              : `Notes not found. Generate Free Notes with AI?`;

          if (confirm(confirmMsg)) {
              if (type === 'PREMIUM' && user.credits < price) {
                  setAlertConfig({isOpen: true, message: `Insufficient Credits! Need ${price} CR.`});
                  return;
              }
              
              if (type === 'PREMIUM' && !user.isAutoDeductEnabled) {
                   // Ideally use modal, but for speed here using simple confirm flow or just proceeding if confirmed
                   // We will assume confirmed above covers intent
              }

              // Deduct Credits if Premium
              if (type === 'PREMIUM' && price > 0 && user.role !== 'ADMIN') {
              ...
              fetchLessonContent(
                  ...
                  chapter, 
                  'English', 
                  type === 'PREMIUM' ? 'NOTES_PREMIUM' : 'NOTES_SIMPLE',
                  0, 
                  type === 'PREMIUM', 
                  15, 
                  promptOverride,
                  ...
                  // Save for future (optional, but good for cache)
                  // We need the key...
                  // const streamKey = ... (replicate key logic)
                  // saveChapterData(key, ...); 
              }).catch(() => {
                  setIsGenerating(false);
                  setAlertConfig({isOpen: true, message: "Generation Failed."});
              });
          }
          return;
      }
  ...
  if (isGenerating) {
      return (
          <div className="flex flex-col items-center justify-center h-screen bg-white">
              ...
              <h3 className="text-xl font-bold text-slate-800 animate-pulse">Generating AI Notes...</h3>
              <p className="text-slate-500">Using {syllabusMode} Syllabus</p>
          </div>
      );
  }
