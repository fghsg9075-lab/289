...
const App: React.FC = () => {
  ...
  const [popupQueue, setPopupQueue] = useState<('STARTUP' | 'TRACKER' | 'CHALLENGE' | 'WELCOME')[]>([]);
  ...
  useEffect(() => {
      ...
      const queue: ('STARTUP' | 'TRACKER' | 'CHALLENGE' | 'WELCOME')[] = [];
      const hasSeenAd = sessionStorage.getItem('nst_ad_seen');
      
      // 1. Startup Ad (Highest Priority, Session Based)
      if (!hasSeenAd && loadedSettings.startupAd?.enabled) {
          queue.push('STARTUP');
      }
  ...
  const handleContentGeneration = async (type: ContentType, count?: number, forcePay: boolean = false) => {
    ...
    try {
        ...
        if (state.selectedClass === 'COMPETITION') {
            promptOverride = s.promptCompetitiveNotes || "";
        } else if (type === 'NOTES_HTML_PREMIUM' || type === 'NOTES_PREMIUM') {
            promptOverride = s.promptPremiumNotes || "";
        } else if (type === 'NOTES_HTML_FREE' || type === 'NOTES_SIMPLE') {
            promptOverride = s.promptFreeNotes || "";
        }
        ...
        const content = await fetchLessonContent(
          ...
          0, false, 15, promptOverride, true 
        );
  ...
  /*
  const handleStartDailyChallenge_OLD = async () => {
       // ... kept for reference if needed, but removing for clean code
  }; 
  */
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white font-sans relative pt-[env(safe-area-inset-top,24px)] pb-[env(safe-area-inset-bottom,32px)]">
      ...
      {popupQueue.length > 0 && (
        <>
            {popupQueue[0] === 'STARTUP' && state.settings.startupAd && (
                <StartupAd 
                    config={state.settings.startupAd} 
                    onClose={() => handlePopupClose('STARTUP')} 
                />
            )}
            ...
            {popupQueue[0] === 'WELCOME' && (
                <WelcomePopup 
                    onStart={() => handlePopupClose('WELCOME')} 
                    isResume={!!state.user} 
                    title={state.settings.welcomeTitle} 
                    message={state.settings.welcomeMessage} 
